...
const SideNavbar = ({ 
  ...
  const { handleApiCall, error: apiError, clearError } = useApiError();
  ...
  useEffect(() => {
      ...
      await handleApiCall(
        ...
        },
        {
          onError: (error) => {
            ...
            // Don't show error for navigation - just use defaults
            setLoading(false);
          },
          onSuccess: () => {
            setLoading(false);
    ...
    const fetchNotificationCount = async () => {
      await handleApiCall(
        async () => {
          console.log('SideNavbar: Fetching notification count...');
        ...
        {
          onError: (error) => {
            console.error('SideNavbar: Error fetching notification count:', error);
  ...
  const workerNavigationLinks = [
    ...
    authenticated 
      ...
      : { icon: LogOut, label: 'Login', action: 'login', danger: false },
  ];
  ...
  const handleLogout = async () => {
    try {
      ...
      // If you have a logout function from useAuth, call it
      if (logout) {
        await logout();
      }
      ...
      <div className={`hidden lg:flex h-full flex-col ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-[#4E6BF5] border-gray-700/50'}`}>
        ...
        <nav className="flex-1 px-4 py-2 space-y-1">
          {navigationLinks.map((link) => {
            ...
            // Handle logout and login differently
            if (link.action === 'logout') {
              return (
                <button
                  key={link.label}
                  onClick={handleLogout}
                  className={`w-full text-left relative flex items-center space-x-3 px-3 py-3 rounded-xl transition-all duration-200 group ${
                    link.danger
                      ? isDarkMode
            ...
            <div className="p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {moreMenuItems.map((item) => {
                  ...
                  // Handle logout differently for mobile
                  ...
                  // Handle login differently for mobile
